...
describe('PlanFreeMinutes UNIT tests', () => {
  ...
  describe('INVALID inputs', () => {
    it.each([[0], [-1], [-10], [-11.11], [1.11], [2.5], [null], [undefined]])(
      'should throw a exception when received values lower than 1 or a float number. value passed %s',
      (invalidFreeMinutes) => {
        //Assert
        expect(() => PlanFreeMinutes.create(invalidFreeMinutes)).toThrow(
          BadRequestException,
        );
      },
    );
